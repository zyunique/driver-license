...
class DBRDrivingLicenseView: UIView {
    ...
    @IBOutlet weak var footView: PopFootView!
    ...
    func initialFromXib(frame: CGRect){
        ...
        self.footView.frame = CGRect(x: 0, y: self.bounds.size.height - SafeAreaBottomHeight - kFootViewHeight, width:  self.contentView.bounds.width, height: kFootViewHeight + SafeAreaBottomHeight)
        self.footView.spentTimeLabel.text = "Total Time Spent: \(barcodeResult.decodeTime) ms"
        ...
        self.footTableView.frame = CGRect(x: 0, y: self.bounds.height - footTableViewHeight - self.footView.bounds.height, width: self.contentView.bounds.width, height: footTableViewHeight)
        ...
        self.drivingLicenseInfTableView.frame = CGRect(x: 0, y: 0, width: self.contentView.bounds.width, height: self.bounds.height - self.footTableView.bounds.height - self.footView.bounds.height)
...
//        self.drivingLicenseInfTableView.scrollToRow(at: IndexPath(item: NSNotFound, section: 0), at: .top, animated: true)
    }
